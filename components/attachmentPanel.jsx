...
import { stopUploadingAnAttachment, uploadAttachmentsThunk, downloadAnAttachmentThunk, stopDownloadingAnAttachment, deleteAnAttachmentThunk, setAttachmentSelectedForDownload } from '../reduxStore/pageSlice';
...
export default function AttachmentPanel({ panelIndex, panel }) {
    ...
    const attachmentPanelSelectedForDownload = useSelector(state => state.page.attachmentPanelSelectedForDownload);
    const saveFileRef = useRef(null);

    const downloadAnAttachmentOnAndroid = (uriString) => {
        debugLog(debugOn, `downloadAnAttachmentOnAndroid ${uriString}`)
        const thisPanel = {
            ...window.bsafesAndroid.panelSelectedForDownload,
            uriString
        }
        dispatch(downloadAnAttachmentThunk({ panel:thisPanel, workspaceKey, uriString }));
        dispatch(setAttachmentSelectedForDownload(null));
    }

    const handleDownload = async () => {
        if (process.env.NEXT_PUBLIC_platform === 'android') {
            window.bsafesAndroid.downloadAnAttachmentOnAndroid = downloadAnAttachmentOnAndroid;
            window.bsafesAndroid.panelSelectedForDownload = panel;
            if(Android){
                console.log("Android.getDownloadedFileUri ...")
                Android.getDownloadedFileUri(panel.fileName, panel.fileType )
            }
            dispatch(setAttachmentSelectedForDownload(panel));
        } else {
    ...
    useEffect(() => {
        if(attachmentPanelSelectedForDownload) {
            //window.bsafesAndroid.downloadAnAttachmentOnAndroid("abc")
        }
    }, [attachmentPanelSelectedForDownload])
