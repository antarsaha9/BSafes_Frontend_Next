...
export function decryptDecode1(content, key, iv) {
    console.log(content, key, iv);
    const encodedCont = decryptBinaryString(forge.util.decode64(content), key, iv);
    console.log(encodedCont,key, iv);
    let cont = DOMPurify.sanitize(forge.util.decodeUtf8(encodedCont));
    console.log(cont);
    return cont;
}
export function decryptDecode2(content, key, iv) {
    console.log(content, key);
    const decodedKey = forge.util.decode64(content);
    const decodedIV = forge.util.decode64(iv);
    const encodedCont = decryptBinaryString(decodedKey, key, decodedIV);
    return encodedCont;
}