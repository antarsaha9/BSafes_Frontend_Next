...
export async function resizeImageFile(file, opts = { maxWidthOrHeight: DEFAULT_MAX_IMAGE_WIDTH_OR_HEIGHT }) {
  debugLog(debugOn, "resizeImageFile");
...
export function downScaleImage(img, exifOrientation, size) {
  return new Promise(async (resolve, reject) => {
    debugLog(debugOn, "downScaleImage");
    const imgCV = document.createElement('canvas');
    const imageWidth = img.naturalWidth;
    const imageHeight = img.naturalHeight;
    const maxCanvasLimit = 4096;
    let canvasWidth, canvasHeight;
    if (imageWidth > maxCanvasLimit || imageHeight > maxCanvasLimit) {
      if (imageWidth > imageHeight) {
        canvasWidth = maxCanvasLimit;
        canvasHeight = imageHeight * (canvasWidth / imageWidth)
      } else {
        canvasHeight = maxCanvasLimit;
    ...
    imgCV.width = canvasWidth;
    imgCV.height = canvasHeight;
    ...
    try {
      ...
      const targetImageWidth = canvas.width;
      const targetImageHeight = canvas.height;
      ...
      if (!byteString) {
        ...
        resolve({ byteString, width: targetImageWidth, height: targetImageHeight });
      }
    } catch (error) {
...
function downScaleCanvas(cv, scale) {
  return new Promise(async (resolve, reject) => {
    debugLog(debugOn, "downScaleCanvas");
    if (!(scale > 0)) {
