...
export function encryptArrayBufferToBinaryStringAsync(arrayBuffer, key) {
  return new Promis(async (resolve, reject) => { 
    ...
    resolve(encryptBinaryString);
  })     
}
  
export function decryptBinaryStringToUinit8ArrayAsync(encryptedString, key) {
  return new Promis(async (resolve, reject) => { 
    let mode = "";
    let header = "";
    for(let i=0; i< 8; i++) {
    ...
    let numberOfSlices = Math.floor(encryptedString/sliceLength) + 1;
