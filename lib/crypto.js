...
export function decryptChunkBinaryStringToUinit8ArrayAsync(encryptedString, key) {
  return new Promise(async (resolve, reject) => { 
    let mode = "";
    let header = "";
    for(let i=0; i< 8; i++) {
    ...
    function decryptASlice(index) {
      return new Promise(async (resolve) => {
        console.log('decrypting slice:', index);
        sliceStr = "";
        if (index === numberOfSlices-1) {
          ...
          decryptedSlice = decryptBinaryString(sliceStr, key);
...
export function decryptChunkBinaryStringToBinaryStringAsync(encryptedString, key) {
  return new Promise(async (resolve, reject) => { 
    let mode = "";
    let header = "";
    let decryptedString = "";
    ...
    function decryptASlice(index) {
      return new Promise(async (resolve) => {
        ...
        if (index === numberOfSlices-1) {
          ...
          decryptedSlice = decryptBinaryString(sliceStr, key);
          decryptedString += decryptedSlice;
          console.log('decryptedSlice length, totalLength:', decryptedSlice.length, decryptedString.length);
        } else {
          end = start + sliceLength;
          for (var offset = start; offset < end; offset++) {
            sliceStr += encryptedString[offset];
