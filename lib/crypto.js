...
export function createAccountRecoveryPhrase(nickname, keyPassword, clientEncryptionKey) {
  const nickNameLength = nickname.length;
  const thisString = `${nickNameLength}:${nickname}${keyPassword}`;
  const accountRecoveryPhrase = encryptBinaryString(thisString, clientEncryptionKey);
  return accountRecoveryPhrase;
} 

export function readAccountRecoveryPhrase(accountRecoveryPhrase, clientEncryptionKey) {
  const thisString = decryptBinaryString(accountRecoveryPhrase, clientEncryptionKey);
