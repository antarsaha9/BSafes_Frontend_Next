...
export function encryptArrayBufferAsync(arrayBuffer, key) {
  return new Promis(async (resolve, reject) => { 
    ...
    const encryptedUint8Array = new Uint8Array(encryptedDataSize);
    
    let sliceLength = __sliceLength;
    let numberOfSlices = Math.floor(arrayBuffer.byteLength/sliceLength) + 1;
    let sliceIndex, start=0, end, sliceStr, offset, encryptedSlice, encryptedStart = 0;;
    ...
    function encryptASlice(index){
      return new Promise((resolve, reject)=> {
        ...
        if(index === numberOfSlices-1){
          ...
          for (offset = encryptedStart; offset < encryptedStart + encryptedSlice.length; offset++) {
          ...
          encryptedSlice = encryptBinaryString(sliceStr, key);
          console.log('encryptedSlice.length:', encryptedSlice.length);
          for (offset = encryptedStart; offset < encryptedStart + __sliceLength + __GCMOverhead; offset++) {
            encryptedUint8Array[offset] = encryptedSlice.charCodeAt(offset - encryptedStart);
          }
