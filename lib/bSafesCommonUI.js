...
export function createANewTeam(teamName, addAction, targetTeam, targetPosition, publicKeyPem) {
  return new Promise(async (resolve, reject) => {
    ...
    const teamIV = forge.random.getBytesSync(16);
    const encodedTeamName = forge.util.encodeUtf8(teamName);
    const encryptedTeamName = encryptBinaryString(encodedTeamName, teamKey, teamIV);
    ...
    const searchKeyEnvelope = encryptBinaryString(searchKey, teamKey, searchKeyIV);
...
export function getTeamName(teamData, privateKey) {
  const teamKeyEnvelope = teamData.teamKeyEnvelope;
  const privateKeyFromPem = forge.pki.privateKeyFromPem(privateKey);
  const encodedTeamKey = privateKeyFromPem.decrypt(forge.util.decode64(teamKeyEnvelope));
  const teamKey = forge.util.decodeUtf8(encodedTeamKey);
  const encryptedTeamName = teamData.team._source.name;
  const teamIV = teamData.team._source.IV;
  const encodedTeamName = decryptBinaryString(forge.util.decode64(encryptedTeamName), teamKey, forge.util.decode64(teamIV));
