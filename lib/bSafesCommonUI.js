...
export async function createANewItem(titleStr, currentContainer, selectedItemType, addAction, targetItem, workspaceKey, searchKey, searchIV) {
    return new Promise( (resolve, reject) => {
      const title = '<h2>' + titleStr + '</h2>';
      const encodedTitle = forge.util.encodeUtf8(title);
    
      const itemKey = setupNewItemKey(workspaceKey);
